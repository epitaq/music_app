// 複雑になりすぎている気がする自分でも管理できていない
// 一つの関数に詰めすぎているから別での利用が難しくなっている
// シャッフルとhtmlの表示が特に
...
function onPlayerStateChange(event){
    ...
    if (event.data==1){
        ...
        playStatus = 1
        // 時間管理スライダーを移動
        changeTimeSlider(videoList[videoIndex])
    } else {
...
// }
function changeTimeSlider(videoData) {
    var nowTime = (player.getCurrentTime() - videoData.start) / (videoData.end - videoData.start)
    // console.log(nowTime)
    document.getElementById('timeSlider').value = nowTime
    setTimeout(changeTimeSlider, 100)
}
...
// videoList をシャッフル
// シャッフル中はtrueそれ以外はfalse onPlayerStateChangeでインデックスを変更＋doneSをfalse
var doneS = false
